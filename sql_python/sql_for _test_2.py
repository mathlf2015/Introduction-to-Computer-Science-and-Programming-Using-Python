...
cnxn = pyodbc.connect('DRIVER={SQL Server};SERVER=192.168.6.88;PORT=1433;DATABASE=ModelFile;UID=sa;PWD=i2mago')
cursor = cnxn.cursor()
#df = cursor.execute("select * from BuyersReview")
#cursor.execute("insert into datatest(a, b) values ('你好', '世界')")
#print(df)
#cnxn.commit()
cursor.execute("select * from ConcernsWordV3")
...
df.to_csv('D:/project_review/concernwords.csv',index=None,encoding='gbk')
print(df.head())
cnxn.commit()
